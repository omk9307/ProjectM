...
# v14.0.0: 동작 학습 관련 메서드들
        ...
        *,
        ...
        try:
            if isinstance(wait_hp_config, dict):
                ...
                try:
                    rec_px = int(wait_hp_config.get('recover_threshold_px', 70))
                except Exception:
                    rec_px = 70
                ...
                self.wait_recover_threshold_px = 70
    ...
    def _maintain_other_player_wait_hold(
        ...
        # [변경] 대기모드 전용 위치 복구 임계값(px) 사용 (기본 70, 0~100 범위)
        try:
            threshold = float(getattr(self, 'wait_recover_threshold_px', 70) or 70)
        except Exception:
            threshold = 70.0
        if threshold < 0.0:
            threshold = 0.0
    ...
    def _start_walk_teleport_tracking(self, start_time: float | None = None):
        ...
        self._walk_teleport_bonus_percent = 0.0
        try:
            interval = max(float(self.cfg_walk_teleport_interval or 0.0), 0.1)
        except Exception:
            interval = 0.5
        # tracking을 갓 시작했을 때 즉시 판정이 가능하도록 직전 판정 시점을 간격만큼 앞당긴다.
        self._last_walk_teleport_check_time = now - interval
