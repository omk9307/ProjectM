...
class MapTab(QWidget):
    ...
    def update_minimap_view(self, q_image, my_player_rects, other_player_rects, primary_target_name):
        ...
        # --- v7.9.0: 지형 및 오브젝트 데이터 렌더링 ---
        # 위치 보정이 성공하여 활성화된 웨이포인트가 있을 때만 그림
        if self.active_waypoints_info and original_w > 0 and original_h > 0:
            # 현재 위치 보정의 기준이 된 핵심 지형 찾기
            # (가장 최근에 보정 로그에 기록된 지형을 기준으로 삼음)
            correction_log = self.correction_log_viewer.toPlainText()
            lines = correction_log.strip().split('\n')
            if lines:
                last_line = lines[-1]
                if "(근거:" in last_line:
                    # --- v7.9.4 수정: 불완전한 try 구문 제거 ---
                    base_feature_id_part = last_line.split('(근거: ')[-1]
                    base_feature_id = base_feature_id_part.split('(')[0].strip()
                    
                    # TODO: 이 base_feature_id를 사용하여 정확한 전역->로컬 좌표 변환 로직 구현 필요.
                    # 현재는 임시로 (0,0)을 기준으로 렌더링합니다.
                    pass

            # 임시 렌더링 (현재는 기능 구현의 기반만 마련, 실제 좌표 변환 로직은 추후 고도화 필요)
            # 지금은 (0,0)을 기준으로 그림
            if self.render_options.get('terrain', True):
                ...
                for line_data in self.geometry_data.get("terrain_lines", []):
                    ...
                    if len(points) >= 2:
                        for i in range(len(points) - 1):
                            # TODO: 이 좌표들을 로컬 좌표로 변환해야 함
                            p1 = QPointF(points[i][0] * scale_x, points[i][1] * scale_y)
                            p2 = QPointF(points[i+1][0] * scale_x, points[i+1][1] * scale_y)
            ...
            if self.render_options.get('objects', True):
                ...
                for obj_data in self.geometry_data.get("transition_objects", []):
                    ...
                    if len(points) == 2:
                        # TODO: 이 좌표들을 로컬 좌표로 변환해야 함
                        p1 = QPointF(points[0][0] * scale_x, points[0][1] * scale_y)
                        p2 = QPointF(points[1][0] * scale_x, points[1][1] * scale_y)
                        painter.drawLine(p1, p2)
                painter.restore()
