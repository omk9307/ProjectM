# map.py
# 2025년 08月 19日 10:00 (KST)
# 작성자: Gemini
# 기능: 실시간 뷰 좌표 변환 로직 완성 및 안정성 강화
# 설명:
...
class MapTab(QWidget):
    ...
    def __init__(self):
        ...
        # --- v8.0.0: 전역 좌표계 정보 저장 변수 추가 ---
        self.global_positions = {}
        # --- v7.9.6 수정: 모든 렌더링 옵션을 포함하도록 확장 ---
        self.render_options = {
            'background': True,
            'features': True,
            'waypoints': True,
    ...
    def load_profile_data(self, profile_name):
        ...
        try:
            ...
            # --- v7.9.6 수정: 렌더링 옵션을 안전하게 불러오기 (하위 호환) ---
            saved_options = config.get('render_options', {})
            # 기본값으로 시작해서, 저장된 값으로 덮어쓰기
            self.render_options = {
    ...
    def update_minimap_view(self, q_image, my_player_rects, other_player_rects, primary_target_name):
        """
        실시간 미니맵 뷰를 업데이트합니다.
        (2025-08-12): 지형/오브젝트 렌더링 시, 탐지된 모든 기준 지형의 오프셋 평균을
                             사용하도록 수정하여 렌더링 떨림 현상을 해결했습니다.
        """
        ...
        if original_w > 0 and original_h > 0:
            ...
            # --- 핵심 지형, 웨이포인트, 플레이어 아이콘 렌더링 (기존과 동일) ---
            # '핵심 지형' 렌더링 옵션 적용
            if self.render_options.get('features', True):
                painter.save()
            ...
            # '웨이포인트' 렌더링 옵션 적용
            if self.active_route_profile_name and self.render_options.get('waypoints', True):
                painter.save()
                current_waypoints = self.route_profiles[self.active_route_profile_name]['waypoints']
                ...
                for name, data in self.active_waypoints_info.items():
                    ...
                    font.setPointSize(14 if is_arrived else 10)
                    painter.setFont(font)
            ...
            # --- [핵심 수정] 지형/오브젝트 렌더링을 위한 안정적인 오프셋 계산 ---
            if self.active_waypoints_info and self.global_positions:
                local_coords_map = {f['id']: f['rect'].topLeft() for f in self.detected_feature_rects}
                
                # 1. 가능한 모든 오프셋을 수집
                offsets = []
                for feature_id, local_pos in local_coords_map.items():
                    if feature_id in self.global_positions:
                        # QPoint와 QPointF의 연산을 위해 타입을 명시적으로 통일
                        offset = QPointF(local_pos) - self.global_positions[feature_id]
                        offsets.append(offset)

                # 2. 수집된 오프셋이 있을 경우, 평균을 계산하여 최종 오프셋으로 사용
                if offsets:
                    ...
                    # 3. 계산된 평균 오프셋을 사용하여 지형 및 오브젝트 렌더링
                        ...
                        for line_data in self.geometry_data.get("terrain_lines", []):
                            ...
                            if len(points) >= 2:
                                for i in range(len(points) - 1):
                                    ...
                                    # 각 점의 로컬 좌표를 계산하고 스케일링
                                    p1_local = (p1_global + final_offset) * scale_x
                                    p2_local = (p2_global + final_offset) * scale_x
    ...
    def _calculate_global_positions(self):
        ...
        # 웨이포인트가 없어도 핵심 지형의 상대 위치는 계산할 수 있어야 함 (향후)
        # 지금은 첫 지형을 원점으로만 설정
