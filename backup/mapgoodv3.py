...
class RealtimeMinimapView(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if render_opts.get('features', True):
            ...
            active_feature_ids = {f['id'] for f in self.active_features}
            ...
            for feature_id, feature_data in self.parent_tab.key_features.items():
                if feature_id in self.parent_tab.global_positions:
                    ...
                    # 1. 모든 지형의 내부는 채우지 않도록 설정
                    painter.setBrush(Qt.BrushStyle.NoBrush)

                    # 2. 탐지 여부에 따라 펜(테두리) 스타일을 결정
                    if feature_id in active_feature_ids:
                        # 감지됨: 파란색 실선
                        pen = QPen(QColor(0, 180, 255), 2, Qt.PenStyle.SolidLine)
                    else:
                        # 감지 안됨: 흰색 점선
                        pen = QPen(Qt.GlobalColor.white, 2, Qt.PenStyle.DashLine)
                    
                    # 3. 결정된 펜으로 사각형 테두리를 그림
                    painter.setPen(pen)
        ...
        if render_opts.get('waypoints', True):
            ...
            # 경로 프로필별로 웨이포인트 순서를 매핑
            wp_order_map = {
                wp['name']: i + 1 
                for route in self.parent_tab.route_profiles.values() 
                for i, wp in enumerate(route.get('waypoints', []))
            }
...
class MapTab(QWidget):
    ...
    def on_detection_ready(self, found_features, my_player_rects, other_player_rects):
        ...
        if not found_features:
            self.minimap_view_label.update()
            return

        # 1. 플레이어의 전역 좌표 계산을 위한 준비
        player_global_positions = []
        
        # 실시간 미니맵 캡처 영역의 중심점 (QPointF로 명시)
        minimap_center_local = QPointF(self.minimap_region['width'] / 2.0, self.minimap_region['height'] / 2.0)

        # 내 캐릭터가 감지되면, 캐릭터의 기준점을 계산
        # 이 기준점은 모든 위치 계산의 중심이 됨
        player_anchor_local = None
        if my_player_rects:
            player_rect = my_player_rects[0]
            # cv2.boundingRect는 (x, y, w, h) 정수 튜플을 반환.
            # bottom 좌표는 y + h. 여기서 오차가 발생하지 않도록 명확히 계산.
            # QPointF를 사용하여 모든 계산을 부동소수점 기반으로 통일.
            # 사각형의 바닥 좌표에 보정 오프셋(PLAYER_Y_OFFSET)을 더해 실제 발 위치에 맞춤
            player_anchor_local = QPointF(player_rect.center().x(), float(player_rect.y() + player_rect.height()) + PLAYER_Y_OFFSET)
        
        # 2. 감지된 각 지형을 기준으로 플레이어의 전역 위치를 추정
        for feature in found_features:
            feature_id = feature['id']
            # feature['local_pos']는 QPoint일 수 있으므로, QPointF로 명시적 변환
            feature_local_pos = QPointF(feature['local_pos'])
            
            if feature_id in self.global_positions:
                # global_positions에 저장된 값은 이미 QPointF임
                feature_global_pos = self.global_positions[feature_id]
                
                # 기준점(플레이어 또는 미니맵 중심)과 탐지된 지형 간의 로컬 오프셋
                # player_anchor_local이 없으면 minimap_center_local을 사용
                ...
                # 플레이어의 전역 좌표 = 기준 지형의 전역 좌표 + 로컬 오프셋
        ...
        self.minimap_view_label.update_view_data(
            ...
            active_features=active_feature_info, # 이 부분은 현재 비어있지만, 필요 시 채워야 함
            my_players=my_player_global_rects,
            other_players=other_player_global_rects
        )
        # QPointF 객체 자체를 보내서 X, Y 혼동을 원천 방지
