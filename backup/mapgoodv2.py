...
class RealtimeMinimapView(QLabel):
    ...
    def paintEvent(self, event):
        ...
        # 카메라의 중심을 기준으로 잘라낼 소스 영역(전역 좌표 기준)
        source_rect = QRectF(
        ...
        # 배경 지도 이미지에서 실제로 잘라낼 영역 (이미지 좌표 기준)
        # 전역 좌표를 이미지 좌표로 변환 (bounding_rect의 시작점만큼 빼줌)
        ...
        # 활성 지형 하이라이트
        painter.save()
        painter.setPen(QPen(QColor(0, 255, 0, 200), 2, Qt.PenStyle.DashLine))
        painter.setBrush(Qt.BrushStyle.NoBrush)
        for feature in self.active_features:
            global_rect = feature['global_rect']
            local_top_left = global_to_local(global_rect.topLeft())
            local_rect = QRectF(local_top_left, global_rect.size() * self.zoom_level)
            painter.drawRect(local_rect)
        painter.restore()
...
class MapTab(QWidget):
    global_pos_updated = pyqtSignal(float, float)
    ...
    def initUI(self):
        ...
        right_layout.addWidget(QLabel("실시간 미니맵 뷰 (휠 스크롤로 확대/축소)"))
        # --- v9.0.0: RealtimeMinimapView 위젯으로 교체 ---
    ...
    def on_player_pos_updated(self, base_feature_id, player_local_y, player_local_x):
        """AnchorDetectionThread에서 플레이어의 로컬 좌표를 받아서 전역 좌표로 변환 후 편집기로 전송"""
    ...
    def on_detection_ready(self, found_features, my_player_rects, other_player_rects):
        ...
        if not found_features:
            self.minimap_view_label.update() # 줌 레벨 변경 등을 반영하기 위해 호출
            return

        # 1. 플레이어의 전역 좌표 계산
        # 여러 지형을 기준으로 계산된 플레이어 위치들의 평균을 내어 정확도 향상
        player_global_positions = []
        
        # 실시간 미니맵 캡처 영역의 중심점
        minimap_center_local = QPointF(self.minimap_region['width'] / 2.0, self.minimap_region['height'] / 2.0)

        # 내 캐릭터가 감지되면, 캐릭터의 중심점을 기준으로 위치 계산
        player_center_local = None
        if my_player_rects:
            # 여러 캐릭터가 감지될 경우, 첫 번째 캐릭터를 기준으로 함
            player_rect = my_player_rects[0]
            player_center_local = QPointF(player_rect.center())
        
        for feature in found_features:
            feature_id = feature['id']
            feature_local_pos = QPointF(feature['local_pos']) # QPointF로 변환
            
            if feature_id in self.global_positions:
                feature_global_pos = self.global_positions[feature_id]
                
                # 기준점(플레이어 또는 미니맵 중심)과 탐지된 지형 간의 로컬 오프셋
                if player_center_local:
                    offset_local = player_center_local - feature_local_pos
                else:
                    offset_local = minimap_center_local - feature_local_pos
                
                # 전역 좌표 = 기준 지형의 전역 좌표 + 로컬 오프셋
        ...
        if my_player_rects:
            # 플레이어의 중심점 오프셋을 이용해 각 사각형의 전역 위치 계산
            player_center_offset = avg_player_global_pos - player_center_local
        ...
        if other_player_rects:
            # 다른 유저들은 탐지된 위치를 기준으로 오프셋을 계산해야 하지만,
            # 여기서는 편의상 내 캐릭터와 동일한 오프셋을 적용 (정확도는 떨어질 수 있음)
            # TODO: 각 유저별로 더 정확한 위치 추정 로직이 필요할 수 있음
            player_center_offset = avg_player_global_pos - (player_center_local if player_center_local else minimap_center_local)
            for rect in other_player_rects:
                other_player_global_rects.append(QRectF(rect).translated(player_center_offset))
        ...
        for feature in found_features:
            ...
            if feature_id in self.global_positions:
                ...
                if feature_data:
                    ...
                    pixmap = QPixmap()
                    pixmap.loadFromData(img_data)
                    global_rect = QRectF(self.global_positions[feature_id], QSizeF(pixmap.size()))
                    active_feature_info.append({'id': feature_id, 'global_rect': global_rect})
        
        # 4. 뷰 업데이트
        self.minimap_view_label.update_view_data(
            camera_center=avg_player_global_pos,
