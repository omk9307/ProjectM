# map.py
# 2025년 08月 18日 19:00 (KST)
# 작성자: Gemini
# 기능: 전체 미니맵 편집기 - 높이 고정 기능 구현
# 설명:
# - v7.6.0: [기능구현] '전체 미니맵 편집기'에 실시간 캐릭터 높이를 기준으로 수평 지형선을
#           그릴 수 있는 '높이 고정' 기능 추가.
#           - [스레드] AnchorDetectionThread에 새로운 시그널(player_y_pos_updated)을 추가하여,
#             탐지된 플레이어의 로컬 Y좌표와 위치 보정의 기준이 된 핵심 지형 ID를 실시간으로 전송.
#           - [좌표계산] MapTab에 전역 Y좌표를 계산하는 로직 추가. 스레드에서 받은 로컬 좌표와
#             편집기에서 계산된 전역 좌표를 결합하여 플레이어의 '전역 Y좌표'를 계산.
#           - [UI/UX] 편집기의 '높이 고정' 체크박스를 활성화하고, MapTab과 시그널-슬롯으로 연결하여
#             계산된 전역 Y좌표를 실시간으로 수신.
#           - [그리기] '높이 고정'이 활성화된 상태에서 지형선을 그릴 때, 마우스 Y좌표 대신
#             수신된 전역 Y좌표를 사용하여 수평선이 그려지도록 그리기 로직 수정.
# - v7.5.5: [버그수정] 지형/오브젝트 삭제 시 화면이 즉시 갱신되지 않는 문제 해결 및 코드 구조 개선.
# - v7.5.2: [기능개선] 오브젝트 데이터에 부모 지형선 ID를 저장하고, 지형선 삭제 시 종속된
#           오브젝트가 연쇄적으로 삭제되도록 기능 개선.
# - v7.5.1: [기능개선] '선택/이동' 및 '오브젝트 추가' 모드에서도 우클릭 삭제가 가능하도록 편의성 향상.
# - v7.5.0: [기능구현] '전체 미니맵 편집기'에 수직 이동 오브젝트(사다리/밧줄)를 그리는 기능 추가.
# - v7.4.2: [기능개선] '보기 옵션'에 지형선 및 층 이동 오브젝트 가시성 제어 체크박스 추가.
# - v7.4.1: [버그수정] 지형 입력 도구의 시작점 스냅 기능 추가 및 미완성 라인 생성 버그 수정.
# - v7.3.0: [기능구현] '전체 미니맵 편집기'에 '지형선'을 그리는 기능 추가.
...
class FullMinimapEditorDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.chk_show_objects.setChecked(True)
...
class AnchorDetectionThread(QThread):
    ...
    # --- v8.0.0 수정: 로컬 좌표 정보를 전달하기 위해 시그널 확장 ---
    correction_status = pyqtSignal(str, str, list, list) 
    features_detected = pyqtSignal(list)
    initial_position_ready = pyqtSignal(dict, tuple)
    # --- v7.6.0: 플레이어 Y좌표 실시간 전송 시그널 추가 ---
    player_pos_updated = pyqtSignal(str, int, int) # Y좌표, X좌표 순
        ...
        with mss.mss() as sct:
            while self.is_running:
                ...
                if self.prev_frame_gray is not None:
                                ...
                                active_waypoints_data, all_found_features, used_features = self.verify_waypoints(curr_frame_gray, player_rect)
                                ...
                                # --- v7.6.0: 플레이어 Y좌표 시그널 발생 로직 ---
                                if player_rect and used_features:
                                    # 가장 신뢰도 높은 기준 지형 하나를 선택
                                    base_feature_id = max(used_features, key=lambda f: f['conf'])['id']
                                    player_bottom_y = player_rect[1] + player_rect[3]
                                    # 변경 전: self.player_y_pos_updated.emit(base_feature_id, player_bottom_y)
                                    
                                    # --- v7.7.0 변경: X좌표 추가 ---
                                    player_center_x = player_rect[0] + player_rect[2] // 2
                                    self.player_pos_updated.emit(base_feature_id, player_bottom_y, player_center_x)
    ...
    def verify_waypoints(self, current_frame_gray, player_rect):
        ...
        all_used_features = [] # --- v7.6.0: 사용된 모든 지형 정보를 수집하기 위한 리스트 ---
        for wp in self.waypoints:
            ...
            if temp_wp.get('key_feature_ids'):
                ...
                if correction_result:
                    ...
                    all_used_features.extend(used_features_with_conf) # --- v7.6.0: 사용된 지형 정보 추가 ---
                    # --- v8.0.0 수정: 로컬 좌표 정보(found_features)를 함께 emit ---
                    self.correction_status.emit(f"'{temp_wp['name']}' 위치 추정 성공!", "blue", used_features_with_conf, found_features)
        ...
        return active_waypoints, found_features_for_vis, all_used_features
...
class MapTab(QWidget):
    # --- v7.6.0: 편집기로 전역 Y좌표를 전달하기 위한 시그널 ---
    global_pos_updated = pyqtSignal(float, float) # Y좌표, X좌표 순
    ...
    def __init__(self):
        ...
        self.editor_dialog = None # --- v7.6.0: 열린 편집기 인스턴스 저장 ---
        # --- v7.9.0: 렌더링 옵션 상태 변수 추가 ---
        self.render_options = {
            'terrain': True,
            'objects': True
        }
        # --- v8.0.0: 좌표 변환을 위한 상태 변수 추가 ---
        self.global_positions = {}
        self.current_anchor_local_coords = {}
    ...
    def load_profile_data(self, profile_name):
        ...
        try:
            ...
            self.key_features = features
            # --- v8.0.0: 보기 옵션 불러오기 ---
            self.render_options = config.get('render_options', {'terrain': True, 'objects': True})
    ...
    def open_full_minimap_editor(self):
        ...
        try:
            ...
            if result:
                # 1. 편집기에서 변경된 데이터를 가져옴
                updated_geometry = self.editor_dialog.get_updated_geometry_data()
                updated_options = self.editor_dialog.get_current_view_options()
                
                # 2. MapTab의 상태 변수를 업데이트
                self.geometry_data = updated_geometry
                self.render_options = updated_options
                self.global_positions = self.editor_dialog.global_positions

                # 3. 업데이트된 상태를 즉시 파일에 저장
                self.save_profile_data()
                self.update_general_log("지형 편집기 변경사항이 저장되었습니다.", "green")
            else:
                self.update_general_log("지형 편집이 취소되었습니다.", "black")
                # 'Cancel' 시에는 아무것도 업데이트하거나 저장하지 않음

        finally:
            self.global_pos_updated.disconnect(self.editor_dialog.update_locked_position)
            self.editor_dialog = None

    def on_player_pos_updated(self, base_feature_id, player_local_y, player_local_x):
        if self.editor_dialog and self.editor_dialog.isVisible():
            if hasattr(self.editor_dialog, 'global_positions'):
                global_positions = self.editor_dialog.global_positions
                
                found_context = False
                for wp in self.editor_dialog.all_waypoints:
                    if any(f['id'] == base_feature_id for f in wp.get('key_feature_ids', [])):
                        if wp['name'] in global_positions:
                            wp_map_origin = global_positions[wp['name']]['map_origin']
                            global_y = wp_map_origin.y() + player_local_y
                            global_x = wp_map_origin.x() + player_local_x
                            self.global_pos_updated.emit(global_y, global_x)
                            found_context = True
                            break
                if not found_context:
                    if base_feature_id in global_positions:
                        feature_global_pos = global_positions[base_feature_id]
                        global_y = feature_global_pos.y() + player_local_y
                        global_x = feature_global_pos.x() + player_local_x
                        self.global_pos_updated.emit(global_y, global_x)
    ...
    def update_correction_log(self, message, color, used_features_with_conf, found_features):
        ...
        # --- v8.0.0: 수신한 로컬 좌표 정보 저장 ---
        for feature in found_features:
            self.current_anchor_local_coords[feature['id']] = feature['rect']
    ...
    def toggle_anchor_detection(self, checked):
        if checked:
            ...
            # --- v7.6.0: 새 시그널 연결 ---
            self.detection_thread.player_pos_updated.connect(self.on_player_pos_updated) # <-- 변경
    ...
    def update_minimap_view(self, q_image, my_player_rects, other_player_rects, primary_target_name):
        ...
        # --- v8.0.0: 지형 및 오브젝트 데이터 최종 렌더링 로직 ---
        if self.global_positions and self.current_anchor_local_coords and original_w > 0 and original_h > 0:
            # 기준 앵커 찾기 (우선순위: P1, P2, ..., 또는 보이는 것 중 아무거나)
            anchor_id = None
            sorted_anchors = sorted(self.current_anchor_local_coords.keys())
            for key in sorted_anchors:
                if key in self.global_positions:
                    anchor_id = key
                    break
            
            if anchor_id:
                global_pos = self.global_positions[anchor_id]
                local_rect = self.current_anchor_local_coords[anchor_id]
                local_pos = QPointF(local_rect.x(), local_rect.y())
                
                # 전역 -> 로컬 변환 오프셋 계산
                offset = local_pos - global_pos
                
                # 렌더링
                if self.render_options.get('terrain', True):
                    painter.save()
                    ...
                    for line_data in self.geometry_data.get("terrain_lines", []):
                        ...
                        if len(points) >= 2:
                            for i in range(len(points) - 1):
                                p1_global = QPointF(points[i][0], points[i][1])
                                p2_global = QPointF(points[i+1][0], points[i+1][1])
                                p1_local = p1_global + offset
                                p2_local = p2_global + offset
                                painter.drawLine(p1_local * scale_x, p2_local * scale_y)
                    painter.restore()

                if self.render_options.get('objects', True):
                    painter.save()
                    painter.setPen(QPen(QColor(255, 165, 0), 3))
                    for obj_data in self.geometry_data.get("transition_objects", []):
                        points = obj_data.get("points", [])
                        if len(points) == 2:
                            p1_global = QPointF(points[0][0], points[0][1])
                            p2_global = QPointF(points[1][0], points[1][1])
                            p1_local = p1_global + offset
                            p2_local = p2_global + offset
                            painter.drawLine(p1_local * scale_x, p2_local * scale_y)
                    painter.restore()
